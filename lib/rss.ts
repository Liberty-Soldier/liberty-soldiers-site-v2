...
async function normalizeFeed(
 ...
 const items = extractItems(feedJson).slice(0, 6);
...
export async function fetchAllHeadlines(): Promise<Headline[]> {
 const feeds = ((NEWS_FEEDS as unknown as FeedInput[]) ?? []).slice(0, 6);
...
export async function fetchNoiseHeadlines(): Promise<Headline[]> {
  const feeds = ((NOISE_FEEDS as unknown as FeedInput[]) ?? []).slice(0, 6);
